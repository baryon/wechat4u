    ...
    value: function syncPolling(callback) {
      ...
      this.syncCheck().then(function (selector) {
        ...
        if (+new Date() - _this2.lastReportTime > 5 * 60 * 1000) {
          debug('Status Report');
          _this2.notifyMobile(_this2.user.UserName);
          _this2.statReport();
          _this2.sendText('心跳：' + new Date().toLocaleString(), 'filehelper');
    ...
    value: function updateContacts(contacts) {
      ...
      contacts.forEach(function (contact) {
        if (_this5.contacts[contact.UserName]) {
          (function () {
            var original = _this5.contacts[contact.UserName];
            _lodash2.default.forEach(contact, function (val, key) {
              if (!original[key]) {
                original[key] = val;
              }
            });
